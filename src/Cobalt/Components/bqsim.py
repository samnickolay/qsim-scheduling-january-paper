    ...
    def _find_job_location_wcheckp_sam_v1(self, args, drain_partitions=set(), taken_partition=set(),
        ...
        for partition in available_partitions:
            ...
            def test_slowdown_threshold(partition_name):
                ...
                # defined at head of file so variables can be accessed by bgsched
...
#            print "hig priority waiting jobs:"
    ...
    # def post_simulation_handling(self,simu_name, simu_tid, checkp_t_internval, dsize_pnode, bw_temp_write):
        ...
        for jobid, job in self.started_job_dict.iteritems():
            ...
            job_intervals = []
            # job_interval_tmp = jobInterval('queued',job.get('submittime'), job_start_times[0][0])
            ...
            if len(job_start_times) != len(all_end_times):
                ...
                print(all_end_times)
                # exit(-1)
            # if len(job_kill_times) != len([tmp_record for tmp_record in job_overhead_records if tmp_record.type == 'restart']):
            #     print(' # kills times != # restart times')
            #     print(job_kill_times)
            #     print('---')
            #     for job_overhead_record in job_overhead_records:
            ...
            #     try:
            #         job_interval_tmp = jobInterval('running', job_start_times[idx][0], all_end_times[idx][0])
            #     except:
            #         pass
            #     job_intervals.append(job_interval_tmp)
            #     if idx < len(all_end_times) - 1:
            #         job_interval_tmp = jobInterval('queued', all_end_times[idx][0], job_start_times[idx+1][0])
            #         job_intervals.append(job_interval_tmp)
            ...
            #     error_margin = 2.0
            #
            #     matching_job_interval_start = None
            #     for job_interval in job_intervals:
            #         if job_interval.start_time - error_margin <= job_overhead_record.start_time:
            #             matching_job_interval_start = job_interval
            #             # break
            #
            #     if abs(job_interval.start_time - job_overhead_record.start_time) <= error_margin:
            #         job_overhead_record.start_time =  job_interval.start_time
            #
            #     matching_job_interval_end = None
            #     for job_interval in job_intervals[::-1]:
            #         if job_interval.end_time + error_margin >= job_overhead_record.end_time:
            #             matching_job_interval_end = job_interval
            #             # break
            #
            #     if abs(job_interval.end_time - job_overhead_record.end_time) <= error_margin:
            #         job_overhead_record.end_time =  job_interval.end_time
            #
            #     if matching_job_interval_start is None and matching_job_interval_end is None:
            #         print("couldn't find correct job_interval for preempt record")
            #         print(job_overhead_record)
            #         exit(-1)
            #
            #     starting_index = job_intervals.index(matching_job_interval_start)
            #
            #     if matching_job_interval_start is None:
            #         matching_job_interval_start = matching_job_interval_end
            #
            #     if matching_job_interval_end is None:
            #         matching_job_interval_end = matching_job_interval_start
            #
            #     while starting_index < len(job_intervals) and job_intervals[starting_index].end_time <= matching_job_interval_end.end_time:
            #         job_intervals.remove(job_intervals[starting_index])
            #
            #     job_interval_tmp = jobInterval(matching_job_interval_start.type, matching_job_interval_start.start_time,
            #                                    job_overhead_record.start_time)
            ...
            #         job_intervals.append(job_interval_tmp)
            #
            #     job_interval_tmp = jobInterval(matching_job_interval_end.type, job_overhead_record.end_time,
            #                                    matching_job_interval_end.end_time)
            #     if get_job_interval_length(job_interval_tmp) > 0:
            #         job_intervals.append(job_interval_tmp)
            #
            #     output_str += '\n' + '-----'
            ...
            for idx, job_interval in enumerate(job_intervals):
                ...
                if job_intervals[idx - 1].type == 'checkpointing' and job_intervals[idx].type == 'checkpointing':
                    ...
                    new_record_type = 'running'
                # elif job_intervals[idx - 1].type == 'checkpointing' and job_intervals[idx].type == 'checkpointing':

                # if job_intervals[idx - 1].end_time in job_start_times:
                #     new_record_type = 'running'
                # else:
                #     new_record_type = 'queued'

                # if job_interval.type in ['waiting', 'restarting']:
                #     new_record_type = 'queued'
                # elif job_interval.type in ['wasted', 'checkpointing']:
                #     new_record_type = 'running'
                # else:
                #     print('invalid job_interval type')
                #     print(job_interval)
                #     exit(-1)
            ...
            # for idx, job_interval in enumerate(job_intervals):
            #     for job_interval2 in job_intervals:
            #         if job_interval == job_interval2:
            #             continue
            #         if job_interval2.start_time == job_interval.start_time and job_interval.end_time <= job_interval2.end_time:
            #             job_interval2.start_time = job_interval.end_time
            ...
            log_run_time = float(job.get('runtime_org'))
            sim_run_time = total_run_time
            run_time_diff = abs(log_run_time - sim_run_time)
            if run_time_diff > 0.0:
                print(('run_time_diff', jobid, log_run_time, sim_run_time, run_time_diff))
            ...
            if run_time_diff > 5.0:
                print ''
                print(('big run_time_diff', jobid, log_run_time, sim_run_time, run_time_diff))
        ...
        for jobid, job_values in jobs_metrics.iteritems():
            ...
            for job_type in trimmed_list:
                experiment_metrics['slowdown_' + job_type].append(job_values['slowdown'])
                ...
                experiment_metrics[job_category + '_slowdown_' + job_type].append(job_values['slowdown'])
