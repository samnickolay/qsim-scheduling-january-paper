...
results_folder = 'results_1-28-19_v3'
...
job_sizes = ['all_', 'narrow_short_', 'narrow_long_', 'wide_short_', 'wide_long_']
job_types = ['rt', 'batch']
...
def main():
    ...
    checkpoint_heuristics_to_spreadsheet = [{'checkpoint_heuristic': 'baseline'},
                                  {'checkpoint_heuristic': 'v2p_app_sam_v1', 'checkpoint_overhead': '5%_overhead', 'runtime_estimator': 'walltime' },
                                  {'checkpoint_heuristic': 'v2p_app_sam_v1', 'checkpoint_overhead': '10%_overhead', 'runtime_estimator': 'walltime' },
                                  {'checkpoint_heuristic': 'v2p_app_sam_v1', 'checkpoint_overhead': '20%_overhead', 'runtime_estimator': 'walltime' },
...
def make_graphs(current_data, data_dicts, data_dicts2=None):
    checkpoint_heuristics_to_graph = [{'checkpoint_heuristic': 'baseline'},
                                      {'checkpoint_heuristic': 'v2p_app_sam_v1', 'checkpoint_overhead': '5%_overhead',
...
def parse_logs(current_data):
    ...
    for log_filename in os.listdir(log_directory):
        if log_filename.endswith(".log") and current_data in log_filename:
...
def make_spreadsheet(data_dicts, checkpoint_heuristics_to_spreadsheet, spreadsheet_metrics):
    ...
    for trimmed in ['', '_trimmed']:
        ...
        for extra_metric in extra_metrics:
            current_line = get_row_values(data_dicts, extra_metric + trimmed, checkpoint_heuristics_to_spreadsheet)
            spreadsheet_lines.append(current_line)

        for job_size in job_sizes:
            spreadsheet_lines.append('')
            for spreadsheet_metric in spreadsheet_metrics:
                current_metric = job_size + spreadsheet_metric + trimmed
